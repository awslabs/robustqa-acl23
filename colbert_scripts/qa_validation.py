...
from util_tokenizers import SimpleTokenizer
...
def calculate_matches(
    ...
    get_score_partial = partial(check_answer, dpr_all_documents=dpr_all_documents, match_type=match_type,
                                tokenizer=tokenizer)
...
def check_answer(questions_answers_docs, dpr_all_documents, tokenizer, match_type) -> List[bool]:
    ...
    for i, doc_id in enumerate(doc_ids):
        doc = dpr_all_documents[str(int(doc_id))]
        text = doc[0] + ' ' + doc[1]
...
def f1_score(prediction, ground_truth):
    ...
    for p in pred:
        if p in gold:
            corr += 1
    prec = corr / len(pred) if len(pred) > 0 else 0.0
    recl = corr / len(gold) if len(gold) > 0 else 0.0

    return 2 * prec * recl / (prec + recl) if prec + recl > 0 else 0.0
